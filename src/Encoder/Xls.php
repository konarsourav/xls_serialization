  ...
  protected function setData(Worksheet $sheet, array $data) {
    foreach ($data as $i => $row) {
      ...
      foreach ($row as $field_name => $value) {
        ...
        $coordinate = Coordinate::stringFromColumnIndex($column) . ($i + 2);
        // Check whether a value is a serialized object of a specific type.
        if (strpos($value, ViewsExcelStyledFieldItemValue::class . '":') !== FALSE) {
          $styled = unserialize($value, [ViewsExcelStyledFieldItemValue::class]);
          if ($styled instanceof ViewsExcelStyledFieldItemValue) {
            $value = $styled->setStyle($sheet, $coordinate);
          }
        }

        $sheet->setCellValue($coordinate, $this->formatValue($value));
        $column++;
      }
    }
  }
